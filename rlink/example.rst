...
We use two image widgets in the WebUI, one to show a plot of sepal length and width and one to show a plot of petal length and width. The file names used by the widgets are retrieved from StringParameters :token:`SepalImg` and :token:`PetalImg`. The image widget searches for these file in the images directory in the "default resource folder of the WebUI":http://manual.aimms.com/webui/aimms4/en/topic/resources. We create a StringParameter :token:`ImgPath` for this:
