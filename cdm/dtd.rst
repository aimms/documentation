...
.. note::
	As of version 24.6.1.1, CDM will automatically create shapshots on checkout for every branch for which no snapshot exists. These snapshots will be updated once a day. In addition, CDM will store the deltas for every commit, which allows a checkout to rely solely on returning a snapshot and a collection of deltas in most cases, thus drastically reducing the need for executing the need to execute expensive SQL queries to create a checkout. 
