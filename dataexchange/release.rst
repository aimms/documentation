...
26.1.5.1 [26-01-2026]
	- Error responses to ``PUT`` and ``DELETE`` file requests were stored in memory streams, but only read from files. You can now configure the prefix where (error) responses will be stored (default memory streams), and they will be read from either memory streams or actual files.

26.1.4.2 [23-01-2026]
	- API calls generated by the OpenAPI code generator would not fail if the DEX library would fail to obtain an OAuth Bearer token
	- When generating the OpenAPI client code with JSON schema explode mode set to 2, the (single) request file reference used in this mode would not be emptied between calls, preventing subsequent POST and PUT requests from succeeding
